...
func SeedCompetitions(db *gorm.DB) error {
	...
	// Get organizer IDs
	var organizers []entity.User
	if err := db.Where("role = ?", "organizer").Find(&organizers).Error; err != nil {
		return err
	}

	if len(organizers) == 0 {
		// If no organizers found, return without error
		return nil
	}
	...
	competitions := []entity.Competition{
		...
		},
		{
			...
			OrganizerID: getOrganizerID(organizers, 1),
...
// Helper function to safely get an organizer ID
func getOrganizerID(organizers []entity.User, index int) uuid.UUID {
	if len(organizers) > index {
